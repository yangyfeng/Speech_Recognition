/**
 * Created by didi hanyang on 2020/06/23.
 *
 * 实时转写调用demo
...
class IatRecorder {
  ...
  wsOpened() {
    if (this.ws.readyState !== 1) {
      ...
      var params = {
        "opt": "START",
        "asr_param": { "middle_result": 1, "rate": 16 }
      };
...
class IatTaste {
  ...
  setResult(data) {
    ...
    var uncertain;
    if (data["asr_content"]) {
      uncertain = data["asr_content"]["uncertaion"];
      certain = data["asr_content"]["nbest"];
    }
    if (certain) {
      certain = certain.replace(/\|/g, '');
      currentText = $('#result_output').html()
    }
